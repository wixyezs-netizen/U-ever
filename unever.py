# bot_aimuniver.py — AIMUNIVER SHOP Cheat Shop
import logging
import asyncio
import aiohttp
import hashlib
...
class Config:
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "8728854452:AAEGsZWyg6c1zMB59vj_ZtkhP4_fDAviePI")
    ...
    SUPPORT_CHAT_USERNAME = os.environ.get("SUPPORT_CHAT_USERNAME", "aimuniver")
...
def generate_license_key(order_id, user_id):
    ...
    return "AIMU-{}-{}-{}-{}".format(h[:4], h[4:8], h[8:12], h[12:16])
...
def create_payment_link(amount, order_id, product_name):
    ...
    success_url = quote('https://t.me/aimuniver_bot?start=success', safe='')
    return (
...
def subscription_keyboard(platform):
    prices = {
        "apk": [
            ("⚡ НЕДЕЛЯ — 170₽", "sub_apk_week"),
            ("🔥 МЕСЯЦ — 350₽", "sub_apk_month"),
            ("💎 НАВСЕГДА — 700₽", "sub_apk_forever"),
        ],
        "ios": [
            ("⚡ НЕДЕЛЯ — 250₽", "sub_ios_week"),
            ("🔥 МЕСЯЦ — 430₽", "sub_ios_month"),
...
def download_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📥 Скачать AimUniver", url=Config.DOWNLOAD_URL)],
        [InlineKeyboardButton(text="💬 Поддержка", url="https://t.me/{}".format(Config.SUPPORT_CHAT_USERNAME))],
        [InlineKeyboardButton(text="🔄 Новая покупка", callback_data="restart")]
    ])
...
async def send_start_message(target, state):
    text = (
        "🎯 <b>AIMUNIVER SHOP — Профессиональный чит для Standoff 2</b>\n\n"
        "✨ <b>Преимущества:</b>\n"
        "🛡️ Надёжная защита от обнаружения\n"
        "🎯 Точный AimBot с тонкой настройкой\n"
        "👁️ WallHack и ESP модули\n"
        "📊 Полное отображение противников\n"
        "⚡ Моментальные обновления\n\n"
        "💬 Покупка / Поддержка: @{support}\n\n"
        "🚀 <b>Нажмите ниже для покупки:</b>"
        ...
        if deep_link.startswith("buy_stars_"):
            ...
            if product:
                ...
                await bot.send_invoice(
                    ...
                    title="AimUniver — {}".format(product['name']),
                    ...
                    start_parameter="aimuniver_payment"
                )
                return
...
async def about_cheat(callback: types.CallbackQuery):
    text = (
        ...
        "🎮 <b>Название:</b> AimUniver\n"
        "🔥 <b>Статус:</b> Актуальная версия\n\n"
        "🛠️ <b>Функции:</b>\n"
        "• 🎯 Профессиональный AimBot\n• 👁️ WallHack\n"
        "• 📍 ESP визуальные эффекты\n• 🗺️ Радар\n"
...
async def process_platform(callback: types.CallbackQuery, state: FSMContext):
    ...
    info = {
        "apk": ("📱 <b>AimUniver Android</b>",
                "• Android 8.0+\n• 1 ГБ оперативной памяти\n• Root не требуется",
                "• APK файл\n• Инструкция\n• Поддержка"),
        "ios": ("🍎 <b>AimUniver iOS</b>",
                "• iOS 14.0+\n• AltStore для установки\n• Jailbreak не требуется",
                "• IPA файл\n• Инструкция\n• Техподдержка")
    }
...
async def process_subscription(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "⭐ Stars: {stars} ⭐\n"
        "💰 GOLD: {gold} 🪙\n\n"
        "💬 Оплата звёздами и голдой: @{support}\n\n"
        "🎯 <b>Способ оплаты:</b>"
    ).format(emoji=product['emoji'], name=product['name'], duration=product['duration'],
             price=product['price'], stars=product['price_stars'],
             gold=product['price_gold'], support=Config.SUPPORT_CHAT_USERNAME)
...
async def process_gold_payment(callback: types.CallbackQuery):
    ...
    chat_message = (
        "Привет! Хочу купить чит AimUniver на Standoff 2. "
...
async def back_to_subscription(callback: types.CallbackQuery, state: FSMContext):
    ...
    titles = {"apk": "📱 <b>AimUniver Android</b>", "ios": "🍎 <b>AimUniver iOS</b>"}
    text = "{}\n\n💰 <b>Выберите тариф:</b>".format(titles.get(platform, "AimUniver"))
    await callback.message.edit_text(text, reply_markup=subscription_keyboard(platform))
    await state.set_state(OrderState.choosing_subscription)
    await callback.answer()
...
async def main():
    logger.info("=" * 50)
    logger.info("AIMUNIVER SHOP BOT")
    logger.info("=" * 50)
    try:
        me = await bot.get_me()
